...
class FSD50K(Dataset):
    ...
    def __getitem__(self, idx):
        if self.split == "train" and idx in self.mixup_indices: # MixUp
            features = torch.Tensor(self.features_getter(features=self.mixup_queue.get())).unsqueeze(0)
            labels = torch.FloatTensor(self.mixup_queue.get())
        else:
            features = torch.Tensor(self.features_getter(idx=idx)).unsqueeze(0)
